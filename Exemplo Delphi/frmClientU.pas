...
type
  ...
  private
    ...
    function GetAssetID   : TConnectorAssetIdentifier; inline;
...
begin
  ...
  if nRes <> NL_OK then
    ...
    begin
      ...
      SetTradeCallbackV2(TradeCallback);
...
begin
  ...
  try
    ...
    cbFunctions.Items.AddObject('GetSerieHistory',              TDLLMarketDataFunctionDetails.Create(MD_HAS_ASSET or MD_HAS_DATE_RANGE, False));
...
function TfrmClient.GetAssetID : TConnectorAssetIdentifier;
begin
  Result.Version := 0;
...
begin
  ...
  try
    ...
    pCancelOrder.AccountID := GetAccountID;
    pCancelOrder.AssetID   := GetAssetID;
    pCancelOrder.Password  := PWideChar(edtPassRot.Text);
...
begin
  ...
  try
    ...
    pZeroPosition.AssetID   := GetAssetID;
  ...
  if Result = NL_OK then
    begin
      ...
      if (Account.AccountFlags and CA_IS_SUB_ACCOUNT) = CA_IS_SUB_ACCOUNT
        then mmUpdates.Lines.Add('GetAccountDetails: ' + Account.BrokerName + ' | ' + Account.OwnerName + ' | ' + Account.SubOwnerName)
        else mmUpdates.Lines.Add('GetAccountDetails: ' + Account.BrokerName + ' | ' + Account.OwnerName);
    end;
end;
...
begin
  ...
  if Result = NL_OK then
    mmUpdates.Lines.Add('GetPositionV2: Qtd=' + Position.OpenQuantity.ToString + ' Price=' + Position.OpenAveragePrice.ToString + ' Side=' + Position.OpenSide.ToString);
end;
...
var
  ...
  dtEnd   : TSystemTime;
const
  c_OrderVersion : Byte = 0; // Versão da estrutura a ser enviada
begin
  AccID := GetAccountID;
...
begin
  ...
  Result := EnumerateAllOrders(@AccID, 0, LPARAM(Self), EnumOrders);
...
begin
  ...
  if ATab = tabMd then
    begin
      ...
      lbTimeStartMd.Enabled  := mdFunction.HasDateRange;
      ...
      lbTimeEndMd.Enabled    := mdFunction.HasDateRange;
...
begin
  ...
  FreeAndNil(FListBuyPriceBook);
  FreeAndNil(FListSellPriceBook);
